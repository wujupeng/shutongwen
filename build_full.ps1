# ShuTongWen IME Build Script - Full Version with All Dependencies

Write-Host "================================================"
Write-Host "Building ShuTongWen IME (Full Build)"
Write-Host "================================================"
Write-Host ""
...
$env:Include = "$vcDir\include;$vcDir\ATLMFC\include;$winSdkDir\include\$winSdkVer\ucrt;$winSdkDir\include\$winSdkVer\shared;$winSdkDir\include\$winSdkVer\um;$winSdkDir\include\$winSdkVer\winrt;$PSScriptRoot\third_party"
...
function Compile-File {
    ...
    $flags = @(
        ...
        "/I""$($PSScriptRoot)\third_party"""
    )
    
    Write-Host "  Compiling: $(Split-Path -Leaf $SourceFile)"
    $result = & cl $flags "/Fo""$OutputObj""" "$SourceFile" 2>&1
...
Write-Host "[Step 1/3] Compiling sqlite3..."
$sqliteObj = Join-Path $objDir "sqlite3.obj"
if (Test-Path "third_party\sqlite3.c") {
    Write-Host "  Compiling: sqlite3.c"
    $result = & cl /c /O2 /MD /W3 /utf-8 "/I""$PSScriptRoot\third_party""" "/Fo""$sqliteObj""" "$($PSScriptRoot)\third_party\sqlite3.c" 2>&1
    if ($LASTEXITCODE -ne 0) {
        $result | Select-String "error" | Select-Object -First 3 | ForEach-Object { Write-Host "    $_" -ForegroundColor Red }
    }
}
...
Write-Host "[Step 2/3] Compiling core modules..."
...
# Link DLL
Write-Host "[Step 3/3] Linking DLL..."
$outputDll = Join-Path $buildDir "ShuTongWenIME.dll"
$objFiles = Get-ChildItem -Path $objDir -Filter "*.obj"

if ($objFiles.Count -gt 0) {
    $objList = ($objFiles.FullName | ForEach-Object { "`"$_`"" }) -join " "
    $linkCommand = "link /DLL /OUT:`"$outputDll`" /INCREMENTAL:NO /SUBSYSTEM:WINDOWS $objList kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib imm32.lib oleacc.lib"
